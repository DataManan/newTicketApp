...
class TicketsBooked(db.Model):
    ...
    showname = db.Column(db.String, db.ForeignKey(
        'shows.show_name'), nullable=False)
    venuename = db.Column(db.String, db.ForeignKey(
        'venues.venue_name'), nullable=False)
    totalticket = db.Column(db.Integer, nullable=False)
    reservation_date = db.Column(DATE(storage_format="%(day)02d/%(month)02d/%(year)04d",
                                      regexp=re.compile("(?P<day>\d+)/(?P<month>\d+)/(?P<year>\d+)")))
    # relationss //many-one//
    venue = db.relationship(
        'Venues', backref=db.backref('ticket_booked', lazy=True))
    show = db.relationship(
        'Shows', backref=db.backref('ticket_booked', lazy=True))
    user = db.relationship(
        'User', backref=db.backref('ticket_booked', lazy=True))
